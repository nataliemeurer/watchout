...
var updateCoords = function(coords, size){
  ...
  return coords.map(function(value, index) {
    value[0] = Math.max(circleR + buffer, (Math.random() * width) - buffer);
    value[1] = Math.max(circleR + buffer, (Math.random() * height) - buffer);
    return value;
  });
}
